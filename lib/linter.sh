...
if [[ -n "${BITBUCKET_CLONE_DIR}" ]]; then
  source /action/lib/functions/bitbucket.sh # Source the function script(s)
elif [[ -n "${CODEBUILD_SRC_DIR}" ]]; then
  source /action/lib/functions/codebuild.sh # Source the function script(s)
fi
...
# Language array #
##################
# FIXME bug? removed csharp dotnet-format not found
