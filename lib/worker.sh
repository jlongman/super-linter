  ...
  if [ ${SKIP_FLAG} -eq 0 ]; then
    ...
    for FILE in "${LIST_FILES[@]}"; do
      ...
      if [ ${ERROR_CODE} -ne 0 ]; then
        ...
        info "To locally re-execute run the following:"
        info "docker run -v \$PWD/$(dirname "${FILE}"):/tmp/lint -e RUN_LOCAL=true -eVALIDATE_${FILE_TYPE}=true  --rm -it jlongman/super-linter:stable"
        error "[${LINT_CMD}]"
